...
KEY_MAP = {
    ...
    r"^output power$|^optical power$|^cw power$|^typ\. power$": "output_power_mw_nominal",
    r"^min\. power$|^power min$": "output_power_mw_min",
    r"^(rms )?noise( \(.*\))?$|^intensity noise$": "rms_noise_pct",
    r"^power stability$|^long-term stability$|^ltp$": "power_stability_pct",
    r"^m.?2$|^m\^2$|^beam quality$": "m2",
    ...
    r"^digital modulation$|^ttl modulation$|^blanking rate$": "modulation_digital_hz",
